...
容器部署：
默认通过本仓库内的 `Dockerfile` + `docker-compose.yml` 本地构建当前代码版本，避免误拉上游镜像导致自定义修改丢失。
...
- 当前仓库内的 [docker-compose.yml](./docker-compose.yml) 会基于当前代码本地构建镜像
- 默认把本地 `./data` 映射到容器 `/app/data`
- 默认端口映射为 `8073:8000`
- 推荐使用 `docker compose up -d --build`，确保服务器拿到的是当前 fork 的最新代码
...
本仓库使用 SemVer 版本号，当前基线版本记录在根目录 `VERSION` 文件中。

- 日常功能修复和兼容性增强先记录到 `CHANGELOG.md` 的 `Unreleased`
- 准备正式发布时，再把 `Unreleased` 整理为对应版本号
- 部署服务器时，优先以 `VERSION` 和 `CHANGELOG.md` 为准确认当前代码状态
